(ns airops.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `airops.route/dispatch`
  が決め、ページの中身は `airops.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  wrangler.jsonc の `main` は `dist/worker.js` を指し、それはこの名前空間を
  コンパイルしたものである。移行前は SvelteKit のビルド出力
  （`svelte/.svelte-kit/cloudflare/_worker.js`、この tree に存在しない）を指して
  いて、読み手が開く `src/app.ts` はどの bundle にも入っていなかった
  （docs/adr/0001、測定は docs/kotoba-layer-audit.md S7）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` / `ongakuka.worker` と同じ約束）。"
  (:require [airops.route :as route]
            [airops.view :as view]
...
(defn- env->map
  ...
  （中継先だけは意図的に出す —— view の docstring を見よ）。"
  [env]
  (if env
...
(defn- proxy-xrpc
  ...
  封筒に包んで `tools/call` で送り、`result` / `structuredContent` を剥がす。"
  [req env nsid]
...
(defn- page-response [env]
  (->response
   (view/render {:css dds-css
                 :routes route/routes
                 :vars (route/env-var-keys (env->map env))
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "air-ops" :runtime "cljs"
