# kotoba/ layer audit — app-air-ops

**この文書は `kotoba/` を測った記録である。appview の移行（docs/adr/0001）とは
別の層を扱う。** 元は `docs/operator-quickstart.md` としてこの位置に置かれていたが、
2026-08-18 の cljs 移行で quickstart は appview のものになったので、まだ真である
測定をここへ移した。

`kotoba/` は 7 ファイル・980 行の TypeScript domain library で、**移行はこれに
1 バイトも触っていない**（理由と測定は `README.md` の「`kotoba/` は移していない」）。
だから以下の §2–§6・§8 は**いまも成り立つ**。

ステップの ✅ は 2026-08-16 に `e0be46d` に対して実際に走らせた結果である。
**移行で偽になった部分は、この冒頭で名指しして訂正してある**（下記）。

## この文書のうち、移行で偽になったもの

| 元の記述 | 訂正（2026-08-18） |
|---|---|
| §0-4「**`.gitignore` が無い**」 | 在る。移行で追加した（`dist/` `.shadow-cljs/` `node_modules/` `.cpcache/` `.wrangler/`） |
| §1「20 upstream blobs, 20 local blobs, zero differences」 | **`e0be46d` 時点の測定**。移行後の tree は upstream と byte 一致ではない。いまも 1 バイトも変わっていないのは**継承 12 ファイル（45,457 バイト）**で、`scripts/verify-docs-claims.cljs` が sha256 で固定している。うち 7 つが `kotoba/` なので、この文書が測った対象は**変わっていない** |
| §7「`main` は SvelteKit のビルド出力」「`src/app.ts` は deploy されていない」 | **これが移行の動機だった。**いま `main` は `dist/worker.js`（`src/airops/` からコンパイル）を指し、`src/app.ts` と `svelte/` は撤去済み。§7 は移行前の測定として残してある（ADR-0001 の証拠） |
| §10「README が .md ではない」 | `README.md` を移行で追加した。`README.edn` の `:canonical-metadata :edn` はそのまま |

そのほか（§2 の走らせ方、§3 の mutant 表、§4–§6 の欠陥、§8 の DNS、§9 の未解決）は
**訂正なしでそのまま成り立つ**。§8 の DNS は 2026-08-18 に再測して同じだった。
...
## §7 ✅ What was deployed BEFORE the migration (kept as ADR-0001's evidence)

> **This section describes the tree at `e0be46d`, before the 2026-08-18 cljs
> migration.** It is kept because it is the measurement ADR-0001 rests on: the
> deployed handler and the file a reader opens were different files. Today
> `wrangler.jsonc`'s `main` is `dist/worker.js`, compiled from `src/airops/`,
> and `src/app.ts` / `svelte/` are gone.
...
## §8 ⚠ Nothing this app declares or calls resolves (re-measured 2026-08-18: unchanged)
...
(§0's fourth hazard — "there is no `.gitignore`" — no longer applies: the migration
added one. Building in the checkout still leaves `node_modules/` and `dist/`, but git
now ignores them.)
